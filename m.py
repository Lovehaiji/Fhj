...
# Join :- https://t.me/+pE_GZwn83y0xZjll # Admin user IDs
admin_id = ["1635908789"]
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"🚀https://t.me/+pE_GZwn83y0xZjll"
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        # Join :- https://t.me/+pE_GZwn83y0xZjll # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- https://t.me/+pE_GZwn83y0xZjll # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 30:
                response = "SHARE KRO FIR WORKING KAREGA https://t.me/+pE_GZwn83y0xZjll."
                bot.reply_to(message, response)
                return
            # Join :- @mklsuraj # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # BY :- @mklsuraj # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  
            time = int(command[3])  
            if time > 240:
...
def send_welcome(message):
    ...
    btn4 = telebot.types.KeyboardButton('/rules ⛳')
...
𝟯. 𝗪𝗲 𝗗𝗮𝗶𝗹𝘆 𝗖𝗵𝗲𝗰𝗸𝘀 𝗧𝗵𝗲 𝗟𝗼𝗴𝘀 𝗦𝗼 𝗙𝗼𝗹𝗹𝗼𝘄 𝘁𝗵𝗲𝘀𝗲 𝗿𝘂𝗹𝗲𝘀 𝘁𝗼 𝗮𝘃𝗼𝗶𝗱 𝗕𝗮𝗻!!@mklsuraj'''
    bot.reply_to(message, response)
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f''' ☑️ 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝙁𝙤𝙧 @mklsuraj 𝙏𝙤 𝙂𝙚𝙩 ??𝙘𝙘𝙚𝙨𝙨 🍾
'''
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗢𝗡𝗟𝗬 𝗢𝗪𝗡𝗘𝗥 𝗖𝗔𝗡 𝗨𝗦𝗘 @mklsuraj."
