    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 30:
                response = "https://t.me/+pE_GZwn83y0xZjll."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # BY :- @mklsuraj # Updated to accept target, time, and port
            ...
            response = "🤦‍♂️https://t.me/+pE_GZwn83y0xZjll"  # Join :- @mklsuraj # Updated command syntax
    else:
        response = " https://t.me/+pE_GZwn83y0xZjll Dm @mklsuraj"
...
def send_welcome(message):
    ...
    btn1 = telebot.types.KeyboardButton('/plan ❄️')
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, 𝗕𝗿𝗼𝘁𝗵𝗲𝗿 𝗢𝗻𝗹𝘆 𝟭 𝗣𝗹𝗮𝗻 𝗜𝘀 𝗣𝗼𝘄𝗲𝗿𝗳𝘂𝗹𝗹 𝗧𝗵𝗲𝗻 𝗔𝗻𝘆 𝗢𝘁𝗵𝗲𝗿 𝗗𝗱𝗼𝘀 @mklsuraj !!:
...
𝗪𝗲𝗲𝗸-->200 𝗥𝘀
𝗠𝗼𝗻𝘁𝗵-->𝟰𝟬𝟬 𝗥𝘀
MAIN CHANNEL https://t.me/+pE_GZwn83y0xZjll Dm @mklsuraj'''
    bot.reply_to(message, response)
